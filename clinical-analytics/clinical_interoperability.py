...
_FHIR_R4_RESOURCE_TYPES: list[str] = [
    "Patient", "Condition", "Observation", "MedicationAdministration",
    "MedicationRequest", "Procedure", "DiagnosticReport", "ImagingStudy",
    "Specimen", "ResearchStudy", "ResearchSubject", "AdverseEvent",
]
...
class InteroperabilityConfig:
    ...
    standards: list[ClinicalStandard] = field(default_factory=lambda: [
        ClinicalStandard.FHIR_R4, ClinicalStandard.ICD_10_CM,
        ClinicalStandard.SNOMED_CT, ClinicalStandard.LOINC,
    ])
...
class ClinicalInteroperabilityEngine:
    ...
    def __init__(
        ...
        logger.info(
            "ClinicalInteroperabilityEngine initialised — site=%s, standards=%s, "
            "ICD-10=%d, LOINC=%d, RxNorm=%d",
            self.site_id, [s.value for s in self.config.standards],
            len(self._icd10_snomed), len(self._loinc_ref), len(self._rxnorm_ref),
        )
    ...
    def _record_audit(self, action: str, details: dict[str, Any]) -> None:
        ...
        self._audit_log.append({
            ...
            "timestamp": time.time(), "site_id": self.site_id,
    ...
    def map_vocabulary(
        self, source_code: str, source_system: ClinicalStandard,
        target_system: ClinicalStandard,
    ) -> VocabularyMapping:
        ...
        """
        self._record_audit("map_vocabulary", {
            "source_code": source_code, "source_system": source_system.value,
        ...
        if source_system == ClinicalStandard.SNOMED_CT and target_system == ClinicalStandard.ICD_10_CM:
            ...
            if icd_codes:
                return VocabularyMapping(
                    source_code=source_code, source_system=source_system,
                    target_code=icd_codes[0], target_system=target_system,
        ...
        if source_system == ClinicalStandard.LOINC:
            ...
            if ref is not None:
                return VocabularyMapping(
                    source_code=source_code, source_system=source_system,
                    target_code=source_code, target_system=target_system,
                    confidence=MappingConfidence.EXACT, display_name=ref.get("display", ""),
                )
        ...
        logger.warning("No mapping found: %s (%s) -> %s",
                        source_code, source_system.value, target_system.value)
        return VocabularyMapping(
            source_code=source_code, source_system=source_system,
            target_code="", target_system=target_system,
    ...
    def _lookup_icd10_to_snomed(self, icd_code: str) -> VocabularyMapping | None:
        ...
        if info is not None:
            return VocabularyMapping(
                source_code=icd_code, source_system=ClinicalStandard.ICD_10_CM,
                target_code=info["snomed"], target_system=ClinicalStandard.SNOMED_CT,
                confidence=MappingConfidence.EXACT, display_name=info.get("display", ""),
            )
        for ref_code, ref_info in self._icd10_snomed.items():
            if ref_code.startswith(icd_code) or icd_code.startswith(ref_code.split(".")[0]):
                return VocabularyMapping(
                    source_code=icd_code, source_system=ClinicalStandard.ICD_10_CM,
                    target_code=ref_info["snomed"], target_system=ClinicalStandard.SNOMED_CT,
                    confidence=MappingConfidence.BROADER, display_name=ref_info.get("display", ""),
                )
        return None

    def _fuzzy_search_all(
        self, query: str, target_system: ClinicalStandard,
        ...
        if best_score >= self.config.fuzzy_threshold:
            ...
            return VocabularyMapping(
                source_code=query, source_system=best_src,
                target_code=best_code, target_system=target_system,
                confidence=conf, display_name=best_display,
            )
        return None

    def align_schemas(
        self, schema_a: dict[str, str], schema_b: dict[str, str],
    ) -> SchemaAlignment:
        ...
        """
        self._record_audit("align_schemas", {
            "schema_a_fields": len(schema_a), "schema_b_fields": len(schema_b),
        })
        ...
        for field_a, desc_a in schema_a.items():
            ...
            for field_b, desc_b in schema_b.items():
                ...
                combined = (0.5 * _normalised_edit_similarity(field_a.lower(), field_b.lower())
        ...
        result = SchemaAlignment(
            source_schema="schema_a", target_schema="schema_b",
        ...
        logger.info("Schema alignment: %d/%d fields mapped, score=%.3f",
                     len(field_mappings), len(schema_a), result.alignment_score)
        return result

    def harmonize_dataset(
        self, data: list[dict[str, Any]],
        source_standard: ClinicalStandard, target_standard: ClinicalStandard,
        ...
        self._record_audit("harmonize_dataset", {
            "record_count": len(data), "source": source_standard.value,
        ...
        logger.info(
            "Harmonized %d records (%s -> %s): mapped=%d, unmapped=%d, "
            "conflicts=%d, quality=%.3f",
            result.records_processed, source_standard.value, target_standard.value,
            result.mapped, result.unmapped, result.conflicts, result.quality_score,
        )
    ...
    def validate_fhir_resource(self, resource: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        self._record_audit("validate_fhir_resource", {
        ...
        if rtype == "Patient":
            ...
            elif resource["status"] not in ("registered", "preliminary", "final", "amended",
        ...
        logger.info("FHIR validation for %s/%s: valid=%s, errors=%d, warnings=%d",
                     rtype, resource.get("id", "?"), is_valid, len(errors), len(warnings))
        return {"valid": is_valid, "resource_type": rtype, "errors": errors,
                "warnings": warnings, "error_count": len(errors), "warning_count": len(warnings)}
    ...
    def map_loinc_to_local(self, loinc_code: str, site_id: str) -> VocabularyMapping:
        ...
        if local_code is not None:
            return VocabularyMapping(
                source_code=loinc_code, source_system=ClinicalStandard.LOINC,
                target_code=local_code, target_system=ClinicalStandard.LOINC,
        ...
        if ref is not None:
            return VocabularyMapping(
                source_code=loinc_code, source_system=ClinicalStandard.LOINC,
                target_code=ref.get("component", loinc_code), target_system=ClinicalStandard.LOINC,
                confidence=MappingConfidence.RELATED, display_name=ref.get("display", ""),
            )
        logger.warning("LOINC code %s not found in reference table", loinc_code)
        return VocabularyMapping(
            source_code=loinc_code, source_system=ClinicalStandard.LOINC,
            target_code="", target_system=ClinicalStandard.LOINC,
            confidence=MappingConfidence.NO_MATCH,
        )
    ...
    def generate_cross_site_alignment_report(
        ...
        for i in range(n):
            for j in range(i + 1, n):
                if matrix[i, j] < 0.40:
                    recs.append(f"Sites {site_ids[i]} and {site_ids[j]} have low alignment "
        ...
        return CrossSiteSchemaReport(
            sites=site_ids, schema_versions={s: f"v1.0-{s}" for s in site_ids},
            alignment_matrix=matrix, recommendations=recs,
        )
    ...
    def _export_sdtm(self, data: list[dict[str, Any]], export_id: str) -> dict[str, Any]:
        ...
        for idx, rec in enumerate(data):
            ...
            subj = _pseudonymise_id(str(rec.get("patient_id", rec.get("subject_id", f"SUBJ-{idx:04d}"))),
                                     self.config.hmac_key)
            dm_rows.append({
                "STUDYID": str(study), "DOMAIN": "DM", "USUBJID": subj, "SUBJID": subj,
                "SITEID": rec.get("site_id", self.site_id), "AGE": str(rec.get("age", "")),
                "AGEU": "YEARS", "SEX": str(rec.get("sex", rec.get("gender", ""))).upper()[:1],
            ...
            if ae_term:
                ae_rows.append({
                    "STUDYID": str(study), "DOMAIN": "AE", "USUBJID": subj,
                    "AETERM": str(ae_term), "AEDECOD": str(rec.get("ae_preferred_term", ae_term)),
                    "AESEV": str(rec.get("ae_severity", "")), "AESER": str(rec.get("ae_serious", "")),
            ...
            if lab_code or lab_val:
                ...
                lb_rows.append({
                    "STUDYID": str(study), "DOMAIN": "LB", "USUBJID": subj,
                    "LBTESTCD": str(li.get("component", lab_code)),
                    "LBTEST": str(li.get("display", lab_code)),
                    "LBORRES": str(lab_val), "LBORRESU": str(rec.get("unit", li.get("unit", ""))),
                    "LBSTRESN": str(lab_val), "LBSTRESU": str(rec.get("unit", li.get("unit", ""))),
        ...
        for name, rows in [("DM", dm_rows), ("AE", ae_rows), ("LB", lb_rows)]:
            if rows:
                ...
                domains[name] = "\n".join(
                    [",".join(hdrs)] + [",".join(r.get(h, "") for h in hdrs) for r in rows])

        logger.info("SDTM export %s: DM=%d, AE=%d, LB=%d", export_id,
                     len(dm_rows), len(ae_rows), len(lb_rows))
        return {"export_id": export_id, "format": DataExchangeFormat.CSV_SDTM.value,
                "domains": list(domains.keys()), "domain_data": domains,
                "row_counts": {"DM": len(dm_rows), "AE": len(ae_rows), "LB": len(lb_rows)},
                "total_rows": len(dm_rows) + len(ae_rows) + len(lb_rows), "timestamp": time.time()}
    ...
    def _export_adam(self, data: list[dict[str, Any]], export_id: str) -> dict[str, Any]:
        ...
        for idx, rec in enumerate(data):
            ...
            age_val = rec.get("age")
            adsl.append({
                "STUDYID": str(study), "USUBJID": subj, "SUBJID": subj,
                "SITEID": rec.get("site_id", self.site_id),
                "AGE": float(age_val) if age_val is not None else np.nan, "AGEU": "YEARS",
            ...
            if ae_term:
                adae.append({
                    "STUDYID": str(study), "USUBJID": subj, "AETERM": str(ae_term),
                    "AEDECOD": str(rec.get("ae_preferred_term", ae_term)),
                    "AESEV": str(rec.get("ae_severity", "")),
                    "AESER": str(rec.get("ae_serious", "")), "TRTEMFL": "Y",
                })

        logger.info("ADaM export %s: ADSL=%d, ADAE=%d", export_id, len(adsl), len(adae))
        return {"export_id": export_id, "format": DataExchangeFormat.PARQUET_ADaM.value,
                "datasets": ["ADSL", "ADAE"], "row_counts": {"ADSL": len(adsl), "ADAE": len(adae)},
                "total_rows": len(adsl) + len(adae), "adsl_data": adsl, "adae_data": adae,
                "timestamp": time.time()}
    ...
    def get_vocabulary_stats(self) -> dict[str, int]:
        ...
        return {"icd10_to_snomed": len(self._icd10_snomed), "loinc_reference": len(self._loinc_ref),
                "rxnorm_reference": len(self._rxnorm_ref), "site_local_lab_maps": len(self._site_local_labs)}
    ...
    def register_custom_mapping(self, icd_code: str, snomed_code: str, display: str) -> None:
        ...
        self._record_audit("register_custom_mapping", {
            "icd_code": icd_code, "snomed_code": snomed_code, "display": display})
    ...
    def register_site_lab_codes(self, site_id: str, loinc_to_local: dict[str, str]) -> None:
        ...
        self._record_audit("register_site_lab_codes", {
            "site_id": site_id, "codes_registered": len(loinc_to_local)})
        logger.info("Registered %d lab code aliases for site %s (total=%d)",
                     len(loinc_to_local), site_id, len(existing))
